...
class TestDidiIndex(unittest.TestCase):
    ...
    def test_didi_index_sma(self):
        test_values = DidiIndex(
            self.source, self.short_length, self.mid_length, self.long_length, ma_method='sma'
        )
    ...
    def test_didi_index_ema(self):
        test_values = DidiIndex(
            self.source, self.short_length, self.mid_length, self.long_length, ma_method='ema'
        )
    ...
    def test_didi_index_rma(self):
        test_values = DidiIndex(
            self.source, self.short_length, self.mid_length, self.long_length, ma_method='rma'
        )
        ...
        ref_values = pd.Series([
            ...
            -14.451138345432298,
            -25.242813194487724
        ], index=range(9, 50), name='RMA')

        pd.testing.assert_series_equal(test_values, ref_values)

    def test_didi_index_dema(self):
        test_values = DidiIndex(
            self.source, self.short_length - 5, self.mid_length - 10, self.long_length - 20, ma_method='dema'
        )
